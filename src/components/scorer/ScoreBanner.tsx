...
export const ScoreBanner: React.FC<ScoreBannerProps> = ({ match, innings }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-[#0e1626] to-[#0a1220] border border-slate-800 p-5 shadow-xl">
      ...
      <div className="absolute bottom-0 left-0 -mb-8 -ml-8 w-48 h-48 rounded-full bg-teal-500/10 blur-3xl pointer-events-none" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        ...
        <div className="md:col-span-6 flex items-baseline space-x-4">
          <div className="font-display text-5xl sm:text-6xl font-black text-white tracking-tight">
            {innings.totalRuns}
            <span className="text-emerald-400 text-4xl sm:text-5xl font-light">
              /{innings.wicketsLost}
            </span>
          </div>

          <div className="text-slate-400 text-sm font-medium">
            <div className="text-lg text-slate-200 font-semibold font-mono">
            ...
            <div className="text-xs text-slate-400">
              CRR: <span className="text-emerald-400 font-semibold font-mono">{crr}</span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-6 flex flex-col justify-center">
          {isChasing ? (
            <div className="bg-slate-950/60 rounded-xl p-3 border border-emerald-500/30 shadow-inner">
              ...
              <div className="text-sm sm:text-base font-bold text-white tracking-wide">
                {matchType === 'TEST' ? (
                  <>
                    Need <span className="text-emerald-400 font-mono text-lg">{runsNeeded}</span> runs to win{' '}
                  ...
                  <>
                    Need <span className="text-emerald-400 font-mono text-lg">{runsNeeded}</span> runs in{' '}
                    <span className="text-amber-300 font-mono text-lg">{ballsRemaining}</span> balls
                  </>
                )}
              </div>
            </div>
          ) : matchType === 'TEST' && (currentInningsIndex === 2 || currentInningsIndex === 3) ? (
            <div className="bg-slate-950/40 rounded-xl p-3 border border-slate-800 flex items-center justify-between">
              <div>
